...
initialize_faction = {
	...
	[_side, _tier call ARWA_ARWA_get_tier_bound] call ARWA_set_kill_count;
	[_side, _tier] call ARWA_set_tier;
	[_side, 0] call ARWA_ARWA_set_tier_progress;	
};
...
calculate_tier_progress = {
	...
	_tier_bound =  if(_tier == 0) then { 0; } else { _tier call ARWA_ARWA_get_tier_bound; };
	_next_tier_bound = (_tier + 1) call ARWA_ARWA_get_tier_bound;
